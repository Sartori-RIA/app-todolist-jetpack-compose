...
interface TodoListDao {
    ...
    @Query("SELECT todo_lists.*, ROUND(CAST(((SELECT COUNT(*) FROM todo_list_items WHERE todo_list_id = :id AND done = 1) * 100.0 / (SELECT COUNT(*) FROM todo_list_items WHERE todo_list_id = :id)) AS FLOAT), 2) AS percentage FROM todo_lists WHERE id = :id LIMIT 1")
